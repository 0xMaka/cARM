...
void main (void) {
  unsigned int a2dc;			//vaiable to store result
  inita2d();
  initLed();
  while(ADCSRA |=  (1 << ADSC)) { 	//wait for conversion to complete
    a2dc = ADCH; 			//*store a2d conversion value
    if (a2dc < 250) { 			//if light level less than
      PORTB |= (1 << PB0);		//light on
    }
    else {
      PORTB &= ~(1 << PB0);		//else light off
    }
  }
}
